...
Windows本体パッケージとは別のモデル自己解凍ZIPとして作成する。利用者はWindows本体の
展開先の親フォルダーを展開先として選択し、`models/muscriptor/`へ配置する。
...
記載した標準GitHub-hosted Windows runnerと`HF_TOKEN` Secretを使用する。
